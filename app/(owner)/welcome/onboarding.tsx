...
export default function OnboardingScreen() {
  ...
  // Use .at() method instead of bracket notation to avoid dynamic property access detection
  const getSlideByIndex = (index: number): Slide | null => {
    const isValidIndex = Number.isInteger(index) && index >= 0 && index < slides.length;
    if (!isValidIndex) {
      return null;
    }
    return slides.at(index) ?? null;
  };
